...
class ContractLine(models.Model):
    ...
    def _last_invoices_self_generated_line(self, limit=1):
        """Return the line(s) generated by self of last non-cancelled `limit` invoice(s)

        Note we cannot query invoice lines directly because we want to order
        by invoice date. So we query the last invoices then filter its lines.
        """
    ...
    def last_invoice_discount_state(self, *discounts):
        """Return if given discounts were applied on last invoice

        The discounts are supposed to be one of current line's applicable discounts.
        ...
        if discounts:
            last_inv_line = self._last_invoices_self_generated_line(1)
            applicable_discounts = list(self._applicable_discount_lines())
            for discount in discounts:
                if discount not in applicable_discounts:
                    raise ValueError(
                        "Given discount %s is not related to current contract line %s"
                        % (discount, self)
                    )
                # NOTE this will soon be improved using a dedicated model change, as
                # detecting a discount was applied by its name is not reasonnable:
                result[discount] = bool(
                    last_inv_line and discount.name in last_inv_line.name
                )
