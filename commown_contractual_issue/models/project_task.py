...
class ProjectTask(models.Model):
    ...
    contractual_issue_date = fields.Date(
        "Issue date", help="Date when the contractual issue occurred"
    )
    penalty_exemption = fields.Boolean(
        "Penalty exemption",
        help="E.g.: customer paid, commercial initiative",
        default=False,
    )
    ...
    require_contract = fields.Boolean(
        "Requires a contract", related="project_id.require_contract"
    )
    ...
    def onchange_partner_id_set_contract(self):
        ...
        if (
            self.contract_id.partner_id.commercial_partner_id
