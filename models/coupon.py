...
COUPON_DEFAULTS = {
    'page_width_mm': 210,
    'page_height_mm': 297,
    'nx': 5, 'ny': 13,      # number of labels in the x and y dirs
    'w': 38.1, 'h': 21.2,   # width and height of the labels (mm)
    'x0': 5.0, 'y0': 11.0,  # pos. of 1st label from sheet corner (mm)
    'dx': 2.5, 'dy': 0.0,   # gap size between rows and columns (mm)
...
def convert_svg_to_pdf(svg_content, settings):
    ...
    try:
        ...
        subprocess.check_call([
            'rsvg-convert', '-f', 'pdf', '-a', '-w', str(width_in_pt),
            '-o', pdf_path, svg_path])
...
class SalePromotionRule(models.Model):
    ...
    def _generate_svg(self):
        ...
        for nb in range(ctx['nx'] * ctx['ny']):
            ...
            ctx.update({'x': ctx['x0'] + i * (ctx['dx'] + ctx['w']),
                        'y': ctx['y0'] + j * (ctx['dy'] + ctx['h'])})
            ctx.update({'cx': ctx['x'] + ctx['w'] / 2,
                        'cy': ctx['y'] + ctx['h'] / 2})
    ...
    def action_generate_and_print_coupons(self):
        ...
        attachment = self.env['ir.attachment'].create({
            'res_model': 'coupon.campaign', 'res_id': self.id,
            'mimetype': mime, 'datas': data.encode('base64'),
            'datas_fname': name, 'name': name,
