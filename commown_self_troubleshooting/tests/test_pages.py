...
def ts_link_urls(doc):
    return set(
        doc.xpath("//a[starts-with(@href, '/page/self-troubleshoot-')]/@href"))
...
class PagesTC(HttpCase):
    ...
    def setUp(self):
        ...
        with self.registry.cursor() as test_cursor:
            ...
            self.product_id = env['product.product'].create({
                'name': 'Test service product', 'type': 'service'}).id
    ...
    def create_contract(self, template, ended=False, in_commitment=True, **kw):
        ...
        cline_attrs = {
            "name": "Line 1", "specific_price": 1., "quantity": 1.,
    ...
    def _ts_page_urls(self, *args):
        ...
        urls = set()
        for arg in args:
            page_ids = self.env["ir.model.data"].search([
                ("model", "=", "website.page"),
