...
function setUpWizard($container) {
    ...
    function setupHumanContactButton(contactStep) {
        function isStepEnabled(wizard, stepIndex) {
            return !wizard.steps.eq(stepIndex).hasClass("disabled");
        }
        ...
        return $("button#contactAHuman").on("click", function (event) {
            ...
            if (!isStepEnabled(wizard, contactStep)) {
                ...
                const step0Contract = $(
                    "#form-step-0 select[name=device_contract]"
                ).val();
                ...
                if (step0Contract) {
                    ...
                    $(
                        "#form-step-" + contactStep + " select[name=device_contract]"
                    ).val(step0Contract);
                }
            } else {
    ...
    $container.on(
        "leaveStep",
        function (e, curStep, curIndex, nextIndex, stepDirection) {
