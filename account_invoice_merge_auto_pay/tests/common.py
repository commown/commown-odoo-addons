...
from odoo.addons.account_invoice_merge_auto.tests.common import AutoMergeInvoiceTC
...
class AutoPayInvoiceTC(AutoMergeInvoiceTC):
    ...
    def _multiple_invoice_merge_test(self, invoices=None, **params):
        ...
        if not invoices:
            invoices = [
                self.create_invoice(self.partner_1, date, **params)
        ...
        new_inv = Invoice.browse(list(merge_infos.keys())[0])
        self.assertEqual(new_inv.date_invoice, fields.Date.from_string("2019-05-16"))
        self.assertEqual({inv.id for inv in invoices}, set(merge_infos[new_inv.id]))
