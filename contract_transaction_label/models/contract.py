...
class Contract(models.Model):
    ...
    def _format_transaction_label(self, invoice, last_date_invoiced):
        ...
        label = label.replace("#START#", invoice.invoice_date.strftime(date_format))
        label = label.replace("#END#", last_date_invoiced.strftime(date_format))
        label = label.replace("#INV#", invoice.name)
        return label
    ...
    def _pay_invoice(self, invoice):
        ...
        if self.transaction_label:
            ...
            _logger.debug("Bank label for invoice %s: %s", invoice.name, label)
