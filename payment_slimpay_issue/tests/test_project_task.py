...
class ProjectTC(TransactionCase):
    ...
    def _execute_cron(self, slimpay_issues, action=None):
        ...
        with mock.patch.object(
                ProjectTask, '_slimpay_payment_issue_fetch',
    ...
    def assertInStage(self, task, ref_name):
        self.assertEqual(list(task.stage_id.get_xml_id().values()),
    ...
    def test_cron_first_issue(self):
        ...
        self.assertEqual(self.invoice.mapped('payment_ids.state'),
                         ['cancelled'])
    ...
    def test_cron_second_issue(self):
        ...
        self.assertEqual(task.invoice_id.mapped('payment_ids.state'),
                         ['cancelled'])
    ...
    def _simulate_wait(self, task, **timedelta_kwargs):
        ...
        task.invoice_next_payment_date = (
            task.invoice_next_payment_date - timedelta(**timedelta_kwargs))
    ...
    def test_functional_3_trials(self):
        ...
        self.assertInStage(task, 'stage_max_trials_reached')
        self.assertEqual(task_emails(task)[0].subject,
                         'YourCompany: max payment trials reached')
