...
class StockMoveTC(SavepointCase):
    ...
    def get_assignments(self, lot, company):
        return self.env["customer_device_manager.device_assignment"].search(
            [("device_id", "=", lot.id), ("partner_id", "=", company.id)],
        )

    def get_visible_assignments(self, lot, user):
        "Return visible assignments for a given user"
        return (
    ...
    def test_device_assignment_lifecycle(self):
        ...
        - Check that the assignment is not visible to the customer and the internal user at the beginning.
        - Send the device.
        - Check that the assignment is visible to the customer and the internal user.
        - Update the assignment as a customer assigner.
        - Receive the device.
        - Check that the assignment is not visible anymore.
        """
        ...
        self.assertFalse(self.get_visible_assignments(self.lot, self.internal_user))
        ...
        self.assertEqual(
            len(self.get_visible_assignments(self.lot, self.customer_device_assigner)),
            1,
        )
